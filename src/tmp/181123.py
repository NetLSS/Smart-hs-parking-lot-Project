# https://stackoverrun.com/ko/q/3128155
...
img_g = cv2.GaussianBlur(img_g,(1,1),0);
...
# 선 탐색
image, coutours, hierachy = cv2.findContours(threshold, cv2.RETR_TREE,
                                             cv2.CHAIN_APPROX_SIMPLE)  # threshold 이미지 (binary 이미지) 를
...
for i in range(len(coutours)):
    ...
    hull.append(cv2.convexHull(coutours[i], False))

# find the main island (biggest area)
cnt = coutours[0]
max_area = cv2.contourArea(cnt)

for cont in coutours:
...
for cnt in coutours:
    cv2.drawContours(img, [cnt], 0, (0, 0, 255),
                     1)  # img 이미지에  cnt (coutours는 Point들의 리스트형태) 좌표에 따라  0,0,255 (GR) 색으로 선 그리기
